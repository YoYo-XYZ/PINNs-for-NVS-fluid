...
class Visualizer(VisualizeTemplate):
        ...
        for i, (key,data) in enumerate(key_cmap_dict.items()):
            ...
            if range_x is None or range_y is None:
                if self.length < 0.001:
                    axes[i].set_xlim(self.X_min - 0.2*self.width, self.X_max + 0.2*self.width)
                    axes[i].set_ylim(self.Y_min, self.Y_max)
                elif self.width < 0.001:
                    axes[i].set_xlim(self.X_min, self.X_max)
                    axes[i].set_ylim(self.Y_min - 0.2*self.length, self.Y_max + 0.2*self.length)
                else:
                    axes[i].set_xlim(self.X_min, self.X_max)
                    axes[i].set_ylim(self.Y_min, self.Y_max)
            else:
    ...
    def plot_distribution(self, key:str):
        fig, ax = plt.subplots()
        ax = plt.subplot()
        self.histplot(self.data_dict[key], ax, f"{key} distribution")
        plt.show()
